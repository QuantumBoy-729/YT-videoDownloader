...
Write-Host "=== Building YouTubeDownloader $Version ===" -ForegroundColor Cyan
...
if ($wixBinPath) {
    ...
    # Use the simple WiX file
    $wxsMain = "installer\product_simple.wxs"
    if (Test-Path $wxsMain -PathType Leaf) {
        try {
            ...
            if (Test-Path $distDir) {
                ...
                & "$wixBinPath\candle.exe" -dVersion=$Version -dDistDir="$distDir" -o installer\ installer\product_simple.wxs installer\files.wxs
                
                # Link MSI
                Write-Host "Linking MSI..." -ForegroundColor Cyan
                & "$wixBinPath\light.exe" installer\product_simple.wixobj installer\files.wixobj -o "YouTubeDownloader-$Version.msi"
                
                if (Test-Path "YouTubeDownloader-$Version.msi") {
                    Write-Host "\nSuccess! Output: YouTubeDownloader-$Version.msi" -ForegroundColor Green
                    Write-Host "File size: $((Get-Item 'YouTubeDownloader-$Version.msi').Length / 1MB) MB" -ForegroundColor Yellow
                } else {
